      ...
      <Link to={`/products/${product.id}`}>
        <div className="bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-500 overflow-hidden border border-gray-100 hover:border-primary-200">
            ...
            <div className="absolute bottom-4 left-4 right-4 opacity-0 group-hover:opacity-100 transform translate-y-4 group-hover:translate-y-0 transition-all duration-300">
              <div className="flex gap-2">
                <Button
                  ...
                  className="flex-1 backdrop-blur-sm bg-white/90 hover:bg-white text-gray-900 border-0 shadow-lg"
                  size="sm"
                >
                ...
                <Button
                  variant="outline"
                  className="backdrop-blur-sm bg-white/90 hover:bg-white border-0 shadow-lg px-3"
                  size="sm"
                >
          ...
          <div className="p-6">
            ...
            {/* Product Name */}
            <h3 className="font-bold text-lg text-gray-900 mb-2 line-clamp-2 group-hover:text-primary-600 transition-colors leading-tight">
              {product.name}
            </h3>
            ...
            <div className="mb-2">
              <div className="flex flex-wrap gap-1">
                {product.scentNotes.top.slice(0, 3).map((note, idx) => (
                  <span
                    key={idx}
                    className="text-xs bg-gradient-to-r from-primary-50 to-purple-50 text-primary-700 px-2 py-1 rounded-full border border-primary-100"
                  >
                    {note}
                  </span>
                ))}
                {product.scentNotes.top.length > 3 && (
                  <span className="text-xs text-gray-500 px-2 py-1">
                    +{product.scentNotes.top.length - 3} more
                  </span>
                )}
              </div>
            </div>
